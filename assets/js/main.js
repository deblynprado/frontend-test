 ...
 */
 function render(item) {
  $.each(item, function( k, e ){
    ...
    var listItem = '\
    <li class="item visible fade" data-pop="'+e.votes+'" data-comments="'+e.comments+'"> \
    ...
    $(".data-list .item").each(function() {
      ...
      if($(this).text().toUpperCase().indexOf(value.toUpperCase()) < 0) {
        ...
        if ((numItems === 0) && (count === 0)) {
          $('main').append('<div class="load-more">Nothing found!</div>');
          count++;
        }
      }
    });
